...
This shows that **there exists an $n_0$, a threshold for $n$, where, we are sure that for any $n$ greater than $n_0$, the inequality $13n+2 \leq 2n^2+n$ is always true**
In this example, the function $2n^2+n$ starts to overtake between $n=6$ and $n=7$.
...
[^4]:
    An algorithm's running time is also dependent on a plethora of other factors such as how the input is initially configured, how fast the processor is, how fast the programing language is and etc.
...
[^5]:
    Don't worry about the how I ended up with these functions.
